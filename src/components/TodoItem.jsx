...
const TodoItem = forwardRef(({ todo, deleteTodo, updateTodo,...props }, ref) => {
  const { id, title, completed } = todo;

  return (
    <article ref={ref} {...props}  className="flex gap-4 border-b  border-b-gray-400">

      <button
        onClick={() => { updateTodo(id) }}
        className={`flex h-5 w-5 flex-none items-center justify-center rounded-full border-2 border-gray-300 ${completed
